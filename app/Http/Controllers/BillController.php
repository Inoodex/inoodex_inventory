    ...
    DB::transaction(function () use ($request, $billData, &$bill, &$challan, $customerId, $clientId, $clientName, $clientAddress, $companyDetail) {
        ...
        if ($request->boolean('auto_generate_challan')) {
            ...
            $challan = Challan::create([
                ...
                'notes' => $request->notes,
                'company_name' => $companyDetail->name ?? (getDefaultCompanyDetail()?->name ?? 'Inoodex Inventory'),
                'signatory_name' => $companyDetail->signatory_name ?? (getDefaultCompanyDetail()?->signatory_name ?? 'Authorized Signatory'),
                'signatory_designation' => $companyDetail->signatory_designation ?? (getDefaultCompanyDetail()?->signatory_designation ?? 'Authorized Person'),
                'company_phone' => $companyDetail->phone ?? (getDefaultCompanyDetail()?->phone ?? '+880 XXXX-XXXXXX'),
                'company_email' => $companyDetail->email ?? (getDefaultCompanyDetail()?->email ?? 'hello@inoodex.com'),
                'company_website' => $companyDetail->website ?? (getDefaultCompanyDetail()?->website ?? 'www.inoodex.com'),
                'show_signature' => $request->boolean('show_signature'),
                'show_seal' => $request->boolean('show_seal'),
            ]);
...
public function show($id)
{
    ...
    $data = [
        ...
        'bank_details' => [
            'account_name' => $bill->bank_account_name ?? (getDefaultCompanyDetail()?->name ?? 'Inoodex Inventory'),
        ...
        'company' => [
            'name' => $bill->company_name ?? (getDefaultCompanyDetail()?->name ?? 'Inoodex Inventory'),
            'signatory_name' => $bill->signatory_name ?? (getDefaultCompanyDetail()?->signatory_name ?? 'Authorized Signatory'),
            'signatory_designation' => $bill->signatory_designation ?? (getDefaultCompanyDetail()?->signatory_designation ?? 'Authorized Person'),
            'phone' => $bill->company_phone ?? (getDefaultCompanyDetail()?->phone ?? '+880 XXXX-XXXXXX'),
            'email' => $bill->company_email ?? (getDefaultCompanyDetail()?->email ?? 'info@inoodex.com'),
            'website' => $bill->company_website ?? (getDefaultCompanyDetail()?->website ?? 'www.inoodex.com'),
        ],
...
public function preview($id)
{
    ...
    $pdfData = [
        ...
        'bank_details' => [
            'account_name' => $bill->bankDetail->account_name ?? (getDefaultCompanyDetail()?->name ?? 'Inoodex Inventory'),
        ...
        'company' => [
            'name' => $bill->companyDetail->name ?? (getDefaultCompanyDetail()?->name ?? 'Inoodex Inventory'),
            'signatory_name' => $bill->companyDetail->signatory_name ?? (getDefaultCompanyDetail()?->signatory_name ?? 'Authorized Signatory'),
            'signatory_designation' => $bill->companyDetail->signatory_designation ?? (getDefaultCompanyDetail()?->signatory_designation ?? 'Authorized Person'),
            'signature_image' => $bill->companyDetail->signature_image ?? null,
            'seal_image' => $bill->companyDetail->seal_image ?? null,
            'phone' => $bill->companyDetail->phone ?? (getDefaultCompanyDetail()?->phone ?? '+880 XXXX-XXXXXX'),
            'email' => $bill->companyDetail->email ?? (getDefaultCompanyDetail()?->email ?? 'info@inoodex.com'),
            'website' => $bill->companyDetail->website ?? (getDefaultCompanyDetail()?->website ?? 'www.inoodex.com'),
