    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $challan = Challan::create([
                ...
                'notes' => $request->notes,
                'company_name' => $request->company_name ?? (getDefaultCompanyDetail()?->name ?? 'Inoodex Inventory'),
                'signatory_name' => $request->signatory_name ?? (getDefaultCompanyDetail()?->signatory_name ?? 'Authorized Signatory'),
                'signatory_designation' => $request->signatory_designation ?? (getDefaultCompanyDetail()?->signatory_designation ?? 'Authorized Person'),
                'company_phone' => $request->company_phone ?? (getDefaultCompanyDetail()?->phone ?? '+880 XXXX-XXXXXX'),
                'company_email' => $request->company_email ?? (getDefaultCompanyDetail()?->email ?? 'hello@inoodex.com'),
                'company_website' => $request->company_website ?? (getDefaultCompanyDetail()?->website ?? 'www.inoodex.com'),
