...
class FrontendController extends Controller
{
    ...
    /**
     * Public Document Verification for QR code scanning.
     * Validates invoices, challans, project bills, warranty claims, and service records.
     */
    public function verifyDocument(string $type, string $code)
    {
        ...
        $docData = [
            ...
            'date' => null,
            'customer' => null,
            'issuer' => getSetting('site_name', config('app.name', 'Inoodex Inventory')),
            'items' => [],
            'financials' => null,
            'raw_document' => null,
            'verified' => false,
        ...
        switch ($type) {
            ...
            case 'sale':
            case 'sales':
                $sale = Sale::with(['customer', 'items.product', 'challan', 'returns'])->where('order_no', $code)->orWhere('id', $code)->first();
                if ($sale) {
                    $docData['verified'] = true;
                    $docData['type'] = 'Sales Invoice';
                    $docData['reference'] = '#' . $sale->order_no;
                    $docData['date'] = $sale->created_at ? $sale->created_at->format('d M Y, h:i A') : 'N/A';
                    $docData['status'] = $sale->status ?? ($sale->due_payment <= 0 ? 'Paid' : ($sale->advanced_payment > 0 ? 'Partial' : 'Pending'));
                    $docData['customer'] = [
                        'name' => $sale->customer?->name ?? 'Walk-in Customer',
                    ...
                    $docData['financials'] = [
                        'total' => $sale->total ?? $sale->payble,
                        'discount' => $sale->discount ?? 0,
                        'tax' => ($sale->vat ?? 0) + ($sale->tax ?? 0),
                        'payable' => $sale->payble,
                        'paid' => $sale->advanced_payment,
                        'due' => $sale->due_payment,
                    ];
                    foreach ($sale->items as $item) {
                        $docData['items'][] = [
                            'name' => $item->product?->name ?? $item->product_name ?? 'Product',
                            'model' => $item->product?->model ?? $item->model ?? null,
                            'quantity' => $item->qty ?? 1,
                            'unit_price' => $item->unit_price ?? $item->price ?? 0,
                            'total_price' => $item->total_price ?? $item->total ?? 0,
                        ];
                    }
                    $docData['raw_document'] = $sale;
                }
                break;

            case 'bill':
            case 'project_bill':
                $bill = \App\Models\ProjectBill::with(['project.client', 'items'])->where('reference_number', $code)->orWhere('bill_number', $code)->orWhere('id', $code)->first();
                if ($bill) {
                    $docData['verified'] = true;
                    $docData['type'] = 'Project Bill';
                    $docData['reference'] = $bill->reference_number ?? $bill->bill_number;
                    $docData['date'] = $bill->bill_date ? \Carbon\Carbon::parse($bill->bill_date)->format('d M Y') : ($bill->created_at?->format('d M Y') ?? 'N/A');
                    $docData['status'] = ucfirst($bill->status ?? 'Active');
                    $docData['customer'] = [
                        'name' => $bill->project?->client?->name ?? 'N/A',
                        'phone' => $bill->project?->client?->phone ?? 'N/A',
                        'address' => $bill->project?->client?->address ?? 'N/A',
                    ];
                    $docData['financials'] = [
                    ...
                    foreach ($bill->items as $item) {
                        $docData['items'][] = [
                            'name' => $item->item_description ?? $item->description ?? 'Bill Item',
                            'model' => null,
                    ...
                    $docData['raw_document'] = $bill;
                }
                break;

            case 'challan':
                $challan = \App\Models\Challan::with(['challanItems', 'customer', 'project'])->where('reference_number', $code)->orWhere('challan_number', $code)->orWhere('id', $code)->first();
                if ($challan) {
                    $docData['verified'] = true;
                    $docData['type'] = 'Delivery Challan';
                    $docData['reference'] = $challan->reference_number ?? $challan->challan_number;
                    $docData['date'] = $challan->challan_date ? \Carbon\Carbon::parse($challan->challan_date)->format('d M Y') : ($challan->created_at?->format('d M Y') ?? 'N/A');
                    $docData['status'] = 'Delivered / Dispatched';
                    $docData['customer'] = [
                        'name' => $challan->recipient_organization ?? $challan->client_name ?? ($challan->customer?->name ?? 'N/A'),
                        'phone' => $challan->company_phone ?? ($challan->customer?->phone ?? 'N/A'),
                        'address' => $challan->recipient_address ?? ($challan->customer?->address ?? 'N/A'),
                    ];
                    ...
                    $docData['raw_document'] = $challan;
                }
                break;

            case 'warranty':
            case 'claim':
                $claim = \App\Models\WarrantyClaim::with(['product', 'customer'])->where('claim_no', $code)->orWhere('id', $code)->first();
                if ($claim) {
                    $docData['verified'] = true;
                    $docData['type'] = 'Warranty Claim';
                    $docData['reference'] = $claim->claim_no;
                    $docData['date'] = $claim->created_at ? $claim->created_at->format('d M Y') : 'N/A';
                    $docData['status'] = ucfirst(str_replace('_', ' ', $claim->status ?? 'Submitted'));
                    $docData['customer'] = [
                        'name' => $claim->customer?->name ?? 'N/A',
                        'phone' => $claim->customer?->phone ?? 'N/A',
                        'address' => $claim->customer?->address ?? 'N/A',
                    ];
                    $docData['items'][] = [
                        'name' => $claim->product?->name ?? 'Product Under Warranty',
                        'model' => $claim->product?->model ?? null,
                        'quantity' => 1,
                        'unit' => 'Unit',
                        'serial' => $claim->product_serial_no ?? 'N/A',
                        'unit_price' => null,
                        'total_price' => null,
                    ];
                    $docData['raw_document'] = $claim;
                }
                break;

            case 'service':
                $service = \App\Models\Service::with(['customer'])->where('service_number', $code)->orWhere('id', $code)->first();
                if ($service) {
                    $docData['verified'] = true;
                    $docData['type'] = 'Service Record';
                    $docData['reference'] = $service->service_number ?? '#' . $service->id;
                    $docData['date'] = $service->created_at ? $service->created_at->format('d M Y') : 'N/A';
                    $docData['status'] = ucfirst($service->status ?? 'Completed');
                    $docData['customer'] = [
                        'name' => $service->customer?->name ?? 'N/A',
                        'phone' => $service->customer?->phone ?? 'N/A',
                        'address' => $service->customer?->address ?? 'N/A',
                    ];
                    $docData['financials'] = [
                        'total' => $service->grand_total ?? $service->total ?? 0,
                        'paid' => $service->paid_amount ?? 0,
                        'due' => $service->due_amount ?? 0,
                    ];
                    $docData['raw_document'] = $service;
                }
                break;
        }

        return view('frontend.pages.verify_document', compact('docData', 'type', 'code'));
    }
}
